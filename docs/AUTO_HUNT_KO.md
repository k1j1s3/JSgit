...
판정 순간의 화면은 `data/bot-evidence`에 저장됩니다. 기본 설정은
`dry_run: true`라서 감지만 하고 귀환 버튼을 누르지 않습니다.
...
좌표 확인 후 `dry_run`을 `false`로 바꾸면 실제 탭을 보냅니다. 잘못된
귀환 슬롯이나 목적지 좌표로 인한 아이템 사용을 막기 위해 검증 전에는
활성화하지 마십시오.
