...
DB_PATH = Path(__file__).with_name("lineager_server_data.sqlite")
...
def handle(client, addr):
    ...
    print("[ACTION] Movement + chat + shop + captured REAL field-monster spawn are enabled. Unknown RPCs will be logged.")
    ...
    print("[ACTION] Look 3 tiles to the right. It should be a living fierce wild boar, not a potion/corpse. Click/attack it once.")
    ...
    while True:
        ...
        if p[0] == 0x0A and len(p) == 10:
            ...
            if target_id == TEST_MONSTER_OBJ_ID:
                print(
                    f"[ATTACK SUCCESS] Client targeted our captured live-monster clone "
                    f"obj_id={TEST_MONSTER_OBJ_ID}. Combat response is the next step."
                )
            elif target_id == 0:
...
def main():
    print(f"[LOCAL SERVER STEP 9E] listening on {HOST}:{PORT}")
    print(f"[LOCAL SERVER STEP 9E] embedded world-init packets={len(WORLD_BURST)}")
    print("[LOCAL SERVER STEP 9E] Real game server is NOT contacted.")
    print()
