...
class DailyQuestRotationTest(unittest.TestCase):
    ...
    def test_alt_skips_normal_clan_quest(self):
        tasks = ROTATION.tasks_for(self.config, "광전1"); self.assertIn("clan_easy", tasks); self.assertIn("clan_hard", tasks); self.assertNotIn("clan_normal", tasks)
    ...
    def test_switch_sequence_uses_recorded_row(self):
        ...
        points = [call.args[2] for call in tap.call_args_list]; self.assertEqual([1232, 635], points[0]); self.assertIn([640, 335], points); self.assertEqual([1085, 660], points[-1])
